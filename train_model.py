...
def evaluate_model(name, model, X_train, y_train_log, X_val, y_val_log, y_val_raw):
    ...
    print(f"R²  : {r2:.4f}")
    mape = mean_absolute_percentage_error(y_val_raw, pred_raw)
    accuracy = 1 - mape
...
df = pd.read_parquet("preprocessed/royalroad_cleaned.parquet")
print("Data Loaded.")

# Split
train_df, val_df = train_test_split(df, test_size=0.1, random_state=5)
